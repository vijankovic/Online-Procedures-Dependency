...
convert_results_sample_size <- function(data) {
  fwer <- data$fwer_mat
  pwr  <- data$pwr_mat
  df   <- as.data.frame(list(as.factor(rep(rownames(fwer),each = ncol(fwer))),as.numeric(rep(colnames(fwer),times = nrow(fwer))),as.numeric(as.vector(t(fwer))),as.numeric(as.vector(t(pwr)))))
  colnames(df) <- c("Method","n","FWER","Power")
  return(df)
}


# generate plot #
plotting_sample_size <- function(data) {
  p1 <- ggplot(data, aes(n, FWER, group = Method, color = Method)) +
    geom_line(aes(linetype = Method)) +
    geom_point() +
    geom_hline(yintercept=0.05, color = "black") +
    xlab("") +
  ...
  p2 <- ggplot(data, aes(n, Power, group = Method, color = Method)) +
    geom_line(aes(linetype = Method)) +
    geom_point() +
    xlab("Sample size") +
    ylab("Power") +
